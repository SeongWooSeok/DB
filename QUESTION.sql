...
-- 32. 부서의 매니저으로 있는 사원의 모든 정보와 부서명을 조회하라.
...
/*계층형 쿼리 : 부모, 자식간의 수직관계를 트리 구조 형태로 보여주는 쿼리
START WITH : 트리 구조의 최상위 행 지정 
CONNECT BY : 부모, 자식의 관계를 지정
PRIOR : CONNECT BY 절에 사용되며 PRIOR에 지정된 컬럼이 맞은편 컬럼 찾아감
CONNECT BY PRIOR 자식컬럼 = 부모컬럼 : 부모 -> 자식 순방향 반대는 반대
*/
...
-- 40. 직무의 종류가 가장 많은 부서의 이름을 조회하라. -- ROWNUM의 문제 중복값이 있을경우 뽑아주지 않음 이럴경우 MAX나 RANK를 써보자
...
-- 69. 회사 전체의 최대 연봉, 최소 연봉, 연봉 총 합 및 평균 연봉을 자연수로 포맷하여 조회한다. TRUNC
...
-- 76. 총 사원 수 및 1995, 1996, 1997, 1998 년도 별 고용된 사원들의 총 수를 다음과 같이 조회한다. 03 04 05 06으로 바꿈 @@@@@@@@
SELECT  (
...
SELECT COUNT(*) FROM EMPLOYEES WHERE TO_CHAR(HIRE_DATE,'YY')='01'; --위에꺼 이런식으로 바꾸면됨
-- 77.  각 부서별 각 직업 별 연봉 총 합을 조회한다. 
...
-- 94. 위치 ID 가 1700 인 사원들의 연봉과 커미션이 동일한 사원들의 LAST_NAME, 부서 번호 및 연봉을 조회한다. 1700으로 해보면 안나옴
