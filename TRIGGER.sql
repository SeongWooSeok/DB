...
RAISE_ APPLICATION_ERROR(사용자 오류 에러)
...
BEGIN
    IF TO_CHAR(SYSDATE,'DY') IN ('토','일','수') THEN 
        IF INSERTING THEN 
            RAISE_APPLICATION_ERROR(-20000, '사원 정보 추가불가');
        ELSIF UPDATING THEN 
            RAISE_APPLICATION_ERROR(-20001, '사원 정보 수정불가');
        ELSIF DELETING THEN 
            RAISE_APPLICATION_ERROR(-20002, '사원 정보 삭제불가');
        ELSE
            RAISE_APPLICATION_DERROR(-20003, '사원 정보 추가불가');
        END IF;
    END IF;
END;
